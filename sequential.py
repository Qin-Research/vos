...
for i in range(1,n_frames):
    ...
    for (j,g) in enumerate(masks):
        ...
        uni = np.unique(segs[j])        
        for u in uni:
            rs, cs = np.nonzero(segs[j] == u)
        
            mean_color = np.mean(im[rs, cs],0)
            mean_vx = np.mean(vx[rs,cs,j])
            mean_vy = np.mean(vy[rs,cs,j])
            mean_x = np.mean(cs)
            mean_y = np.mean(rs)
    ...
    dim = 3
    fg_samples = np.array(fg_colors).reshape(-1,3)        
    bg_samples = np.array(bg_colors).reshape(-1,3)
...
#        colors += [mean_color[0], mean_color[1], mean_color[2], mean_x, mean_y]    
    ...
    for u in uni:
        rs, cs = np.nonzero(segs[i] == u)
        prob_im[rs,cs] = forest_proba[u,0]
        prob_im2[rs,cs] =  forest_proba[u,1]
    ...
    pair_features = pair_feature[index:index+len(np.unique(segs[i]))]
                                                                           
    index +=  len(np.unique(segs[i]))  
    n_nodes = colors.shape[0]

    assert(n_nodes == pair_features.shape[0])
