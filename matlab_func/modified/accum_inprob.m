...
function [ result, tags ] = accum_inprob(flow, superpixels, diffused)
...
%      output{ 1 } = double(gt_ratio);
 %   foreGain{ 1 } = zeros( length( gt_ratio' ), 1 );
    for( frame = 1: frames - 1 )

        % Find number of connections between superpixels
        connect = connectivity{ frame };
%        connect = connectivity{ frame } .* edge_strength{frame};
...
%        tags{ i } = spixelRatio{ i }  + backGain{ i };
