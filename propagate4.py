...
def get_edge_strength(frames, segs,lam=0.05):
    ...
    for i in range(n_frames):
        ...
        color = np.zeros((len(uni), 3))
        for u in uni:
            rows, cols = np.nonzero(segs[i] == u)
            color[u] = np.mean(im[rows, cols], axis= 0)
    ...
    for i  in range(n_frames - 1):
        d.append(np.exp(-lam * cdist(colors[i], colors[i+1])))
...
unary = loadmat("../FastVideoSegment/unary2_%s.mat" % name)['unaryPotentials']
